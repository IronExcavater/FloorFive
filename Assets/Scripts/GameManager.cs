...
public class GameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (_instance == null)
        {
            ...
            // Null 체크
            if (roomPrefab == null || roomPrefab.Count == 0)
                Debug.LogError("roomPrefab이 할당되지 않았습니다!");
            if (checkpointPrefab == null || checkpointPrefab.Count == 0)
                Debug.LogError("checkpointPrefab이 할당되지 않았습니다!");
            if (levelTrans == null)
                Debug.LogError("levelTrans가 할당되지 않았습니다!");
    ...
    public static void CreateArea(Area area, Connector connector, GameManager _instance)
    {
        ...
        if (isCheckpoint)
        {
            newArea = _instance._checkpointPool.Get();
        }
        else
        {
            newArea = _instance._roomPool.Get();
        }

        // 상태 초기화 필요 (예: newArea.ResetState() 등)
        if (newArea is IResettable resettable)
